...
def save_course_goal(request):
    ...
    if COURSE_GOALS_NUMBER_OF_DAYS_GOALS.is_enabled():
        ...
        if days_per_week is None or subscribed_to_goal_reminders is None:
            raise ParseError("'days_per_week' and 'subscribed_to_goal_reminders' are required.")

        try:
            add_course_goal(request.user, course_id, days_per_week, subscribed_to_goal_reminders)
            return Response({
